...
import { getNetworkStateInfo, getNetworkTypeInfo } from '../config';

defineProps<{
  isSelected?: boolean;
  item: IotNetCompApi.IotNetComp;
}>();
    ...
    <div class="mt-4 flex flex-col gap-2.5">
      ...
      <div class="flex items-center justify-between">
        ...
        <div
          ...
          :class="[
            getNetworkStateInfo(item.state).statusColor === 'success'
        ...
        >
          ...
          ></div>
          {{ getNetworkStateInfo(item.state).label }}
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-3 border-t border-border pt-3">
        ...
        <div class="flex flex-col gap-1">
          ...
          >
            {{ getNetworkTypeInfo(item.type).label }}
          </span>
        </div>
        <!-- Address -->
        <div class="flex flex-col items-end gap-1">
          ...
          <span
            v-if="item.addressInfo && item.addressInfo.length > 0"
            class="truncate font-mono text-[10px] font-medium text-foreground/80"
          >
            {{ item.addressInfo[0]?.address }}
          </span>
          <span v-else class="text-[10px] italic text-muted-foreground/40">
            -
          </span>
        </div>
      </div>
