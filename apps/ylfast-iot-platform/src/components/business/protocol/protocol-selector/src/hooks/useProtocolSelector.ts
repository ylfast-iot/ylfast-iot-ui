...
import ProtocolSelector from '../index.vue';
...
export function useProtocolSelector(
  ...
  const methods: SelectorActionType<IotProtocolApi.Protocol> = {
    getSelection(): IotProtocolApi.Protocol[] {
  ...
  const ConnectedProtocolSelector = defineComponent({
    name: 'ConnectedProtocolSelector',
    setup() {
      return () =>
        h(ProtocolSelector, {
          ref: selectorRef,
  ...
  return [ConnectedProtocolSelector, methods] as const;
}
