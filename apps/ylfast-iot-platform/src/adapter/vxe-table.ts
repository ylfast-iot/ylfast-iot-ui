...
function createCustomRender(renderOpts: any, params: any) {
  const { props } = renderOpts;
  const component =
    typeof props?.renderComponent === 'function'
      ? props.renderComponent(params)
      : (typeof props?.component === 'string'
          ? getGlobalComponent(props?.component)
  ...
  const modelValuePropName = props?.modelValuePropName || 'value';
  const updateModelValueEventName = `onUpdate:${modelValuePropName}`;
  // 解决字段为多层级
  const defaultProps = {
    [modelValuePropName]: getValue(column.field as string, row),
    [updateModelValueEventName]: (val: any) => {
      setValue(column.field as string, val, row);
    },
  };
  return h(
    component,
...
setupVbenVxeTable({
  configVxeTable: (vxeUI) => {
    ...
    vxeUI.renderer.add('CustomRender', {
      renderTableDefault: createCustomRender,
      renderTableEdit: createCustomRender,
      renderTableCell: createCustomRender,
    });
    ...
    vxeUI.renderer.add('AInputNumber', {
      renderTableEdit(renderOpts, renderParams) {
        const { props } = renderOpts;
        const { column, row } = renderParams;
