...
async function validate() {
  ...
  if (invalidNodes.length > 0) {
    if (invalidNodes.length > 0) {
      message.error(
        $t('network.clusterConfig.validateError', {
  ...
  // Then validate all nested network configurations
...
onMounted(async () => {
  ...
  try {
    ...
    clusterNodes.value = (res.nodes || []).map((node) => ({
...
<template>
  <div class="w-full">
    ...
    <div v-if="items.length === 0" class="py-10 text-center">
      <Empty :description="$t('network.clusterConfig.empty')">
