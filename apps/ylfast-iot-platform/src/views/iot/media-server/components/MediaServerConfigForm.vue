...
import { getMediaServerConfigMetadata } from '#/api/iot/media-server';
...
watch(
  ...
  async (newProvider) => {
    if (newProvider) {
      loading.value = true;
      try {
        const res = await getMediaServerConfigMetadata(newProvider);
        metadata.value = Array.isArray(res) ? res : [res];
      } catch (error) {
        console.error('Failed to load media server config metadata:', error);
        metadata.value = [];
      } finally {
        loading.value = false;
      }
  ...
  <div class="w-full">
    ...
    >
      ...
      <div class="text-sm font-medium text-slate-500 dark:text-gray-400">
        {{ $t('common.placeholder.select') }}{{ $t('mediaServer.provider') }}
      </div>
      <div class="mt-1 text-[11px] text-slate-400 dark:text-gray-500">
        {{ $t('mediaServer.tips.selectProviderFirst') }}
      </div>
    </div>
  </div>
</template>
