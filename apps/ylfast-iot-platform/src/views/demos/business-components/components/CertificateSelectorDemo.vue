...
import {
  CertificateSelector,
  useCertificateCardSelector,
  useCertificateListSelector,
  useCertificateSelector,
} from '#/components/business/certificate/certificate-selector';
...
// --- 3. Hook 弹窗 ---
const [CertificateSelectorModal, { open }] = useCertificateSelector();
...
// --- 4. 嵌入式 Hook (列表/卡片) ---
const [CertificateList, listApi] = useCertificateListSelector({
  showSearchForm: false, // 隐藏搜索
  showPager: false, // 隐藏分页
  multiple: true,
});

const [CertificateCard, cardApi] = useCertificateCardSelector({
  ...
  <div class="p-4">
    <Card title="证书选择器组件演示">
      <Tabs v-model:active-key="activeKey">
          ...
          <div class="flex h-[200px] flex-col items-center justify-center p-4">
            <Space direction="vertical" align="center">
              <div class="text-gray-500">
                使用 useCertificateSelector Hook 命令式唤起弹窗
              </div>
            ...
            <!-- 必须挂载 Modal 组件 -->
            <CertificateSelectorModal hide-trigger />
          </div>
        </TabPane>
          ...
          <div class="grid grid-cols-1 gap-4 p-4 lg:grid-cols-2">
            ...
            <div class="flex flex-col rounded-lg border border-gray-200 p-3">
              <div class="mb-3 flex items-center justify-between">
                <Tag color="blue">纯列表 (CertificateListSelector)</Tag>
                <Space>
              ...
              >
                <!-- 这里不需要传 display-mode，因为组件本身就是 List -->
                <CertificateList />
              </div>
            </div>
            ...
            <div class="flex flex-col rounded-lg border border-gray-200 p-3">
              <div class="mb-3 flex items-center justify-between">
                <Tag color="green">纯卡片 (CertificateCardSelector)</Tag>
                <Space>
                  <Button size="small" @click="handleCardSelect">
                    获取选中
                  </Button>
                  <Button size="small" @click="clearCardSelection">清空</Button>
                </Space>
              </div>
              <div
                class="flex-1 overflow-hidden border border-gray-100 bg-white"
        ...
        <!-- Tab 4: 卡片模式 -->
        <TabPane key="card" tab="卡片模式 (Card Mode)">
          <div class="space-y-4 p-4">
            <div class="font-bold">弹窗卡片模式 (Change display-mode)</div>
            <CertificateSelector
              v-model:value="cardValue"
              display-mode="card"
              @change="handleChange"
            />
            <div class="text-xs text-gray-500">Selected: {{ cardValue }}</div>
          </div>
        </TabPane>
      </Tabs>
    </Card>
