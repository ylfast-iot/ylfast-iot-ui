...
import {
  ...
  useNotifyTemplateCardSelector,
  useNotifyTemplateListSelector,
  useNotifyTemplateSelector,
} from '#/components/business/notify/template-selector';
...
// --- 2. Hook 弹窗 ---
const [TemplateSelectorModal, { open }] = useNotifyTemplateSelector();
...
// --- 3. 嵌入式 Hook (列表) ---
const [TemplateList, listApi] = useNotifyTemplateListSelector({
  showSearchForm: true,
  multiple: true,
});
...
// --- 4. 嵌入式 Hook (卡片) ---
const [TemplateCard, cardApi] = useNotifyTemplateCardSelector({
  multiple: true,
  showSearchForm: true,
});
        ...
        <TabPane key="basic" tab="基础用法 (Declarative)">
          <div class="grid grid-cols-1 gap-6 p-4 md:grid-cols-2">
            ...
            <div class="space-y-2">
              ...
              <NotifyTemplateSelector v-model:value="multiValue" disabled />
            </div>
          </div>
        </TabPane>
          ...
          <div class="flex h-[200px] flex-col items-center justify-center p-4">
            <Space direction="vertical" align="center">
              <div class="text-muted-foreground">
                使用 useNotifyTemplateSelector Hook 唤起弹窗
              </div>
