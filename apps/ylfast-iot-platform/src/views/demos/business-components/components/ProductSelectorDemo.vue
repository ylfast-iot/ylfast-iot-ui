...
import {
  ProductSelector,
  useProductCardSelector,
  useProductListSelector,
  useProductSelector,
} from '#/components/business/product/product-selector';
...
// --- 3. Hook 弹窗 ---
const [ProductSelectorModal, { open }] = useProductSelector();
...
async function handleOpenHook() {
  ...
  if (rows && rows.length > 0) {
    hookResult.value = `选中: ${rows[0].productName} (${value})`;
...
// --- 4. 嵌入式 Hook (列表) ---
const [ProductList, listApi] = useProductListSelector({
...
// --- 5. 嵌入式 Hook (卡片) ---
const [ProductCard, cardApi] = useProductCardSelector({
  multiple: true,
  showSearchForm: true,
  showPager: true,
});
          ...
          <div class="flex h-[200px] flex-col items-center justify-center p-4">
            <Space direction="vertical" align="center">
              <div class="text-gray-500">
                使用 useProductSelector Hook 命令式唤起弹窗
              </div>
            ...
            <!-- 必须挂载 Modal 组件 -->
            <ProductSelectorModal hide-trigger />
          </div>
        </TabPane>
