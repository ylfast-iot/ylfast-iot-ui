...
import {
  ProtocolSelector,
  useProtocolCardSelector,
  useProtocolListSelector,
  useProtocolSelector,
} from '#/components/business/protocol/protocol-selector';

const activeKey = ref('basic');

// --- 1. 基础用法 ---
const multiValue = ref<string[]>([]);
const singleValue = ref<string>('java-protocol'); // Example ID
...
// --- 3. Hook 弹窗 ---
const [ProtocolSelectorModal, { open }] = useProtocolSelector();
...
async function handleOpenHook() {
  ...
  if (rows && rows.length > 0) {
    hookResult.value = `选中: ${rows[0].protocolName} (${value})`;
...
// --- 4. 嵌入式 Hook (列表) ---
const [ProtocolList, listApi] = useProtocolListSelector({
...
// --- 5. 嵌入式 Hook (卡片) ---
const [ProtocolCard, cardApi] = useProtocolCardSelector({
  multiple: true,
  showSearchForm: true,
  showPager: true,
});
...
<template>
  <div class="p-4">
    <Card title="协议选择器组件演示 (Protocol Selector)">
          ...
          <div class="flex h-[200px] flex-col items-center justify-center p-4">
            <Space direction="vertical" align="center">
              <div class="text-gray-500">
                使用 useProtocolSelector Hook 命令式唤起弹窗
              </div>
            ...
            <!-- 必须挂载 Modal 组件 -->
            <ProtocolSelectorModal hide-trigger />
          </div>
        </TabPane>
