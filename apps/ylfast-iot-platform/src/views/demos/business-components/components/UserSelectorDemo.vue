...
import {
  ...
  useUserListSelector,
  useUserSelector,
} from '#/components/business/user/user-selector';

const activeKey = ref('basic');

// --- 1. 基础用法 ---
const multiValue = ref<string[]>([]);
const singleValue = ref<string>('admin');
...
// --- 3. Hook 弹窗 ---
const [UserSelectorModal, selectorApi] = useUserSelector();
const hookResult = ref('');

async function handleOpenHook() {
  const result = await selectorApi.open?.({
...
// --- 4. 嵌入式 Hook (列表) ---
const [UserList, listApi] = useUserListSelector({
...
// --- 5. 嵌入式 Hook (卡片) ---
const [UserCard, cardApi] = useUserCardSelector({
  multiple: true,
  showSearchForm: true,
  showPager: true,
});
        ...
        <TabPane key="basic" tab="基础用法 (Declarative)">
          <div class="grid grid-cols-1 gap-6 p-4 md:grid-cols-2">
            <div class="space-y-2">
              ...
              <UserSelector v-model:value="multiValue" @change="handleChange" />
          ...
          <div class="flex h-[200px] flex-col items-center justify-center p-4">
            <Space direction="vertical" align="center">
              <div class="text-gray-500">
                使用 useUserSelector Hook 命令式唤起弹窗
              </div>
            ...
            <!-- 必须挂载 Modal 组件 -->
            <UserSelectorModal hide-trigger />
          </div>
        </TabPane>
