...
import {
  DeviceSelector,
  useDeviceCardSelector,
  useDeviceListSelector,
  useDeviceSelector,
} from '#/components/business/device/device-selector';

const activeKey = ref('basic');

// --- 1. 基础用法 ---
const multiValue = ref<string[]>([]);
const singleValue = ref<string>('2007743719858302976');
...
// --- 3. Hook 弹窗 ---
const [DeviceSelectorModal, { open }] = useDeviceSelector();
...
async function handleOpenHook() {
  ...
  if (rows && rows.length > 0) {
    hookResult.value = `选中: ${rows[0].deviceName} (${value})`;
...
// --- 4. 嵌入式 Hook (列表) ---
const [DeviceList, listApi] = useDeviceListSelector({
...
// --- 5. 嵌入式 Hook (卡片) ---
const [DeviceCard, cardApi] = useDeviceCardSelector({
  multiple: true,
  showSearchForm: true,
  showPager: true,
});
              ...
              <DeviceSelector v-model:value="customTriggerValue">
                <template #trigger="{ handleOpen }">
                  <Button type="primary" @click="handleOpen">
                    点击选择设备 ({{ customTriggerValue.length }})
                  </Button>
                </template>
              </DeviceSelector>
            </div>
          ...
          <div class="flex h-[200px] flex-col items-center justify-center p-4">
            <Space direction="vertical" align="center">
              <div class="text-gray-500">
                使用 useDeviceSelector Hook 命令式唤起弹窗
              </div>
            ...
            <!-- 必须挂载 Modal 组件 -->
            <DeviceSelectorModal hide-trigger />
          </div>
        </TabPane>
